...
namespace QuickUnity.Editor.Menu
{
    public static class CreateUnityConstantsMenu
    {
        private static readonly string RootPath =
            Path.Combine(Application.dataPath, "App/auto_generated");
        ...
        private static void Execute(MenuCommand cmd)
        {
            ...
            try
            {
                ...
                builder.AppendLine();
                builder.Indent1().AppendLine("public static class Constants");
                builder.Indent1().AppendLine("{");

                builder.AppendLine(GenerateScript("LayerName", builder =>
                {
                    LayerNameStringBuilder(builder);
                    LayerMaskStringBuilder(builder);
                }));

                builder.AppendLine(GenerateScript("SortingLayerName", SortingLayerStringBuilder));
                builder.AppendLine(GenerateScript("PrefabInResourcesPath", PrefabPathStringBuilder));
                builder.AppendLine(GenerateScript("SceneName", SceneBuildIndexStringBuilder));
                builder.AppendLine(GenerateScript("TagName", TagStringBuilder));

                builder.Indent1().AppendLine("}");
                builder.AppendLine("}");

                ExIO.WriteAllText(RootPath, "Constants_AutoGenerated.cs", builder.ToString());
            }
            catch (Exception e)
            {
        ...
        private static string GenerateScript(string className, Action<StringBuilder> action)
        {
            StringBuilder builder = new StringBuilder();
            builder.Indent2().AppendFormat($"public static class {className}").AppendLine();
        ...
        {
            builder.Indent3().AppendLine("// Layer");
            ...
            {
                builder.Indent3().AppendFormat(@"public const int {0} = {1};", n.var, n.val).AppendLine();
            }
        }
        ...
        private static void LayerMaskStringBuilder(StringBuilder builder)
        {
            ...
            builder.Indent3().AppendLine("// LayerMask");
            foreach (var n in InternalEditorUtility.layers.Select(c => new
            {
                var = RemoveInvalidChars(c),
            ...
            {
                builder.Indent3().AppendFormat(@"public const int {0}Mask = {1};", n.var, n.val).AppendLine();
            }
        }
        ...
        private static void SortingLayerStringBuilder(StringBuilder builder)
        {
            ...
            foreach (var keyValuePair in sortingLayerNameDic)
            {
                ...
                builder.Indent3().AppendLine($"public const string {key} = \"{name}\";");
            }
        }
        ...
        private static void PrefabPathStringBuilder(StringBuilder builder)
        {
            ...
            foreach (var asset in assets)
            {
                ...
                // Exclude assets that are not included in the Resources folder.
                ...
                builder.Indent3().AppendFormat(@"public const string {0} = ""{1}"";", RemoveInvalidChars(variableName),
        ...
        private static void SceneBuildIndexStringBuilder(StringBuilder builder)
        {
            ...
            for (var i = 0; i < scenes.Length; i++)
            {
                ...
                builder.Indent3().AppendLine($"public const string {sceneName} = \"{sceneName}\";");
            }
        }
        ...
        private static void TagStringBuilder(StringBuilder builder)
        {
            ...
            foreach (var tag in tags)
            {
                ...
                builder.Indent3().AppendLine($@"public const string {name} = ""{name}"";");
            }
        }
